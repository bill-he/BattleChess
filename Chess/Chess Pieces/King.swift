...
class King: ChessPiece {
    ...
    func validCastlingMove(_ startRookX: Int, endRookX: Int, startEmptyX: Int, endEmptyX: Int, y: Int, movementLeft: Bool, board: [[BoardSpace]]) -> Bool{
        if let castlingRook = board[startRookX][y].occupyingPiece{
            if(parseHorizontal(startEmptyX, endPos: endEmptyX, destinationY: y, movementLeft: movementLeft, board: board) && castlingRook.isKind(of: Rook.self) && !(castlingRook as! Rook).moved && castlingRook.pieceColor == pieceColor){
